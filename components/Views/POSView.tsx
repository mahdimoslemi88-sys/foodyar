...
export const POSView: React.FC = () => {
  ...
  // Optimization: Memoize categories to prevent recalculation on every render
  const categories: string[] = useMemo(() => ['همه', ...new Set<string>(activeMenu.map((m: MenuItem) => m.category))], [activeMenu]);

  const filteredMenu = useMemo(() => {
    const query = searchQuery.toLowerCase();
    return activeMenu
      .filter(m => (selectedCategory === 'همه' || m.category === selectedCategory) && m.name.toLowerCase().includes(query));
