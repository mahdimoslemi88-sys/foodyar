...
const createSystemInstruction = (state: FullState) => {
  ...
  // BOLT OPTIMIZATION: Pre-index for O(1) lookups in loops
  ...
  salesLast7Days.forEach(sale => {
      sale.items.forEach(item => {
          ...
          if (menuItem) {
              ...
              // Passing maps for O(1) lookup inside calculateRecipeCost
...
export const generateMenuEngineeringAnalysis = async (
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Aggregate sales count in a single pass instead of nested loops
    const salesCountMap = new Map<string, number>();
    sales.forEach(sale => {
        sale.items.forEach(item => {
            salesCountMap.set(item.menuItemId, (salesCountMap.get(item.menuItemId) || 0) + item.quantity);
        });
    });
    ...
    const salesData = menu.map(item => {
      const salesCount = salesCountMap.get(item.id) || 0;
